...
def get_prompt_text(name: str, args: dict) -> str:
    if name == "statcan-data-lookup":
        ...
        return (
            ...
            "\n"
            "Step 2 — Explore structure and build the --key filter:\n"
            "  statcan metadata <product-id>\n"
            "  → Note the dimension positions and the member IDs you want (e.g. '1.1.1').\n"
            "\n"
            "Step 3 — Download data (--key is required):\n"
            '  statcan download <product-id> --key "1.1.1" --last 12 --output ./data.csv\n'
            "  head -2 ./data.csv    # column names and dimension positions\n"
            "\n"
        ...
        return (
            ...
            '  statcan download <product-id> --key "1.1.1" --last 5 --dry-run   # preview SDMX URL before fetching\n'
            "\n"
        ...
        return (
            ...
            "━━━ Claude Code (bash sandbox) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "\n"
            "# 1. Inspect dimensions to build the --key filter\n"
            f"statcan metadata {pid}\n"
            "#    → Note the member ID you want at each dimension position (e.g. 1.1.1)\n"
            "\n"
            "# 2. Download data (replace 1.1.1 with the key you built in step 1)\n"
            f'statcan download {pid} --key "1.1.1" --last {last_n} --output {out}\n'
            "\n"
            "# 3. Inspect column layout (note column numbers for filtering)\n"
            f"head -2 {out}\n"
            "\n"
            "# 3. Row count\n"
            f"awk 'NR>1' {out} | wc -l\n"
        ...
        return (
            ...
            "━━━ Claude Code (bash sandbox) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "\n"
            "# 1. Check table structure (dimensions, member IDs, member counts)\n"
            f"statcan metadata {pid}\n"
            "#    → Read the dimension positions and member IDs. Build a --key like '1.1.1'.\n"
            "\n"
            "# 2. Sample 3 most recent periods with a focused key to see column layout\n"
            f'statcan download {pid} --key "1.1.1" --last 3 --output {sample_out}\n'
            f"head -2 {sample_out}             # column names and positions\n"
            f"awk 'NR>1' {sample_out} | wc -l  # rows in 3-period sample\n"
            "\n"
            "# 3. Unique values in each dimension of the sample (adapt column numbers from step 2)\n"
            ...
            "#    Widen or narrow --key to hit the size you want.\n"
            "\n"
            "# 5. Preview SDMX URL without downloading\n"
            f'statcan download {pid} --key "1.1.1" --last 5 --dry-run\n'
            "\n"
            "# 6. Full download with the key you built\n"
